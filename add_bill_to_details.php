...
if(!isset($_COOKIE["PHPSESSID"]))
{
	...
	if (isset($_POST['submit']) && ($_POST['submit'] == "Add")) 
	{
		...
		else
		{
			...
			else 
			{
				...
				echo $ins_sql="insert into GST_BILL_TO_DETAILS values(CURRENT_TIMESTAMP,'$bill_id','".$_POST['bill_gstin']."','$bill_pan','".$_POST['bill_name']."','".str_replace("'", "", $_POST['bill_address'])."','".$_POST['bill_district']."','".$_POST['bill_pin']."','".$_POST['bill_state']."','$state_code','".$_POST['bill_tan']."','".$_POST['bill_contact_person']."','".$_POST['bill_contact']."','".$_POST['bill_email']."','".$_SESSION["username"]."','".$_POST['tax_type']."')";
				odbc_close_all();
				$sqlconnect=odbc_connect($dsn,$username,$password) or die("ODBC Connection Failed");
	...
	<form name='add_bill_to_details' action='add_bill_to_details.php' method='POST'  onSubmit="return validate()">
		...
		<table style="background-color:#CCCC99" width="100%" border="1" >				
				...
				<tr>
				<th><div align="right"><span style="color:#663300">GSTIN : <span style="color: #FF0000; font-weight: bold">*</span></span><br /><br /></div></th>
				<th align="left"><input type="text" name="bill_gstin" value="<?php if(isset($_POST['bill_gstin'])) echo $_POST['bill_gstin']; else echo $_SESSION['gstin_pan'] ?>"   />
					<br /> Ex : 33AAAAA1111A1Z5

				</th>
				</tr>
				<tr>
				<th><div align="right"><span style="color:#663300">TAX Payer Type :</span></div></th>
				<th align="left">
					<select name='tax_type'>
						<?php 
							if(isset($_POST['tax_type'])){ ?> <option value="<?php echo $_POST['tax_type']; ?>"><?php echo $_POST['tax_type'];  ?></option>
						<?php } ?>
